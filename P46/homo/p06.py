...
even='07-2018 july odd numbers.xls'
...
def px(name):
    """
    this load as excel fill and returns the data 
    """
    df = pd.read_excel(name, header=19)
    newdf=df.drop(df.columns[[1,2,3,4,6,8,9,11,12,14,15,16,17,19,20,22,23,25,26,28,29,31,32]], axis=1, inplace=True)  
    newdf=df.fillna('*')
...
def load(df):
    ...
    #quit()
    '''
        Rank Categ/Subcat  sales     rec_retail    rec_cost        venret  \
    0      *            *  Sales  Cost of sales  Cost recvd  Retail recvd
    3     11       04/010      *              *           *             *
    5      *            *   2984           1388        2175          5238
    8      8       04/012      *              *           *             *
    10     *            *   3446           1516         528          1311
    13    88       04/014      *              *           *             *
    15     *            *      0              0           0             0
    18    29       04/016      *              *           *             *
    20     *            *    894            417         219           512
    23    16       04/018      *              *           *             *
    25     *            *   1771            796         793          2024
    28    26       04/020      *              *           *             *
    30     *            *    941            448         124           330
    33    30       04/022      *              *           *             *
    35     *            *    889            373          88           198
    ...
    '''
    print '+++' 
    #sales,rec_cost,rec_retail,venret,coo,roo,inv,mup,mdown='','','','','','','','',''
    new_index='*'
    for i in df.iterrows():
        ...
        if new_index !='*':
            ...
            print 'i[1]["sales"]        :',i[1]["sales"]
            print 'float(i[1]["sales"]  :',float(i[1]["sales"])
            print i[1]
            output[key]["sales"]      += float(i[1]["sales"])
            output[key]["rec_cost"]   += float(i[1]["rec_cost"])            
            output[key]["rec_retail"] += float(i[1]["rec_retail"])
            output[key]["venret"]     += float(i[1]["venret"])
            output[key]["coo"][0]        += float(i[1]["coo"])            
            output[key]["roo"][0]        += float(i[1]["roo"])
    ...
    print 'output=', output
    print ''
    print output['001_04_12']
    print
    print output['001_04_22']
#df=px4(even)
df=px(even)
load(df)
